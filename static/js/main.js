...
jQuery(document).ready(function(jQuery) {
	...
	jQuery('.site-header .search-trigger').click(function(){
		if( jQuery('.site-header .search-form').hasClass('hide') )
		{
			/* hide search field */
			jQuery('.site-header .search-form').removeClass('hide');
			jQuery('.site-header .search-field').focus();
			jQuery(this).html('<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>');
			return false;
		}
		else
		{
			/* show search field */
			jQuery('.site-header .search-form').addClass('hide');
			jQuery(this).html('<span class="glyphicon glyphicon-search" aria-hidden="true"></span>');
			return false;
		}
	});
		...
		jQuery.ajax({
			...
			success: function(data) {
				...
				if (parent != '0') {
					...
					jQuery('.comment-respond').before('<ol class="commentlist">' + data + '</ol>');// your comments wrapper
				}
	...
	addComment = {
		...
		createButterbar: function(message) {
			var t = this;
			t.clearButterbar();
			jQuery("body").append('<div class="butterBar text-center"><span class="btn btn-default">' + message + '</span></div>');
...
jQuery(document).on("click", "#fa-loadmore", function() {
	var _self = jQuery(this),
		_postlistWrap = jQuery('.post-archives'),
		_button = jQuery('#fa-loadmore'),
		_data = _self.data();
	if (_self.hasClass('is-loading')) {
		return false
	} else {
		_button.html('<i class="fa fa-spinner fa-pulse"></i>加载中 ...');
