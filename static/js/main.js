...
jQuery(document).ready(function(jQuery) {
	...
	jQuery('.site-header .search-trigger').click(function(){
		if( jQuery('#search-panel').hasClass('open') ){
			/* hide search field */
			jQuery('#search-panel').removeClass('open');
		}else{
			jQuery('#search-panel').addClass('open');
			jQuery('.site-header .search-field').focus();
		}
	});
	jQuery("#site-navigation").on("click", function(event) {
        if (jQuery(event.target).is(".nav-close") || jQuery(event.target).is("#site-navigation")) {
            event.preventDefault();
            jQuery(this).removeClass("open");
        }
    });
	jQuery("#search-panel").on("click", function(event) {
        if (jQuery(event.target).is(".search-close") || jQuery(event.target).is("#search-panel")) {
            event.preventDefault();
            jQuery(this).removeClass("open");
        }
    });
	jQuery("#toggle-login").on("click", function() {
		if( jQuery('#login-form').hasClass('open') ){
			/* hide search field */
	...
	jQuery(document).on("click", "#fa-loadmore", function() {
		...
		if (_self.hasClass('is-loading')) {
			...
			_button.html('<i class="iconfont icon-refresh fa-spin"></i> 加载中 ...');
